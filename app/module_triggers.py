...
triggers = [
  ...
  (0, 0, 0,
    [
      ...
      (key_is_down, key_left_control),
      # test
      # (display_message, "@{!}I'm centring cam on the fleet party!^If any error messages appears after this message or the fleet is visible, then please report immediately!"),
    ...
    [
      ...
      (try_begin),
        ...
        (try_for_range, ":faction", npc_kingdoms_begin, npc_kingdoms_end),
          (faction_get_slot, ":marshal", ":faction", slot_faction_marshal),
          (gt, ":marshal", 0),
          (str_store_troop_name, s0, ":marshal"),
          (str_store_faction_name, s1, ":faction"),
          (troop_get_slot, ":cur_troop_party", ":marshal", slot_troop_leaded_party),
          (try_begin),
            (ge, ":cur_troop_party", 0),
            (party_is_active, ":cur_troop_party"),
            (party_get_position, pos0, ":cur_troop_party"),
            (position_get_x, reg0, pos0),
            (position_get_y, reg1, pos0),
            (party_get_slot, reg2, ":cur_troop_party", slot_party_ai_state),
            (party_get_slot, ":object", ":cur_troop_party", slot_party_ai_object),
            (try_begin),
              (gt, ":object", 0),
              (str_store_party_name, s2, ":object"),
              (display_message, "@{!}Marshal {s0} of the {s1} at {reg0},{reg1} state {reg2} object {s2}."),
            (else_try),
              (display_message, "@{!}Marshal {s0} of the {s1} at {reg0},{reg1} state {reg2}."),
            (try_end),
          (else_try),
            (display_message, "@{!}Marshal {s0} of the {s1} has no party."),
          (try_end),
        (try_end),
      (try_end),
      ...
      (try_begin),
        ...
        (try_for_range, ":home", spawn_points_begin, spawn_points_end),
          ...
          #TEST:
          # (assign, reg7, "$g_lord_hide"),
          # (display_message, "@g_lord_hide was {reg7} ..."),
          # (try_begin),
          # (le, "$g_lord_hide", 0),
          ...
          # (end_try),
          # (assign, reg7, "$g_lord_hide"),
          # (display_message, "@..and is {reg7} now."),
          #TEST END
